...
def start_gui():
    ...
    def __on_recover(operation_log: HistoryLog.HistoryLog):
        public_resources.current_image_class.layers[public_resources.current_image_class.active_layer][
            0] = operation_log.image
        operation_log = None

    def __on_new_log(__history_log):
        customtkinter.CTkButton(frame, text=__history_log.operation_name, command=__create_command(__history_log)).pack(
            fill='x', pady='2')
    ...
    def __force_history_refresh():
        ...
        for __history_log in public_resources.current_image_class.image_operations_history:
            customtkinter.CTkButton(frame, text=__history_log.operation_name,
                                    command=__create_command(__history_log)).pack(fill='x', pady='2')
