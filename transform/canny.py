...
def start_gui():
    ...
    def __on_value_change(event=None):
        if not preview.get():
            return
        output = cv2.Canny(image_copy_uint, threshold1.get(), threshold2.get())
